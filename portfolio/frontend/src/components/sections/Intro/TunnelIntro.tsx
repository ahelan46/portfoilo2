...
const COLORS = ["#00F0FF", "#00FF7F", "#FF007F", "#7F00FF", "#FFD700", "#FF4500"];
...
export default function TunnelIntro({ text = "AHELAN" }: { text?: string }) {
  ...
  useEffect(() => {
    ...
    const buildMask = () => {
      ...
      if (!ctx) return;
      ctx.scale(dpr, dpr);
      const family = '"Arial Black", "Arial Bold", Gadget, sans-serif';
      let size = h * 0.62;
