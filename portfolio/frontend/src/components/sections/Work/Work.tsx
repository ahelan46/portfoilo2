...
export default function Work({ projects = PROJECTS }: { projects?: any[] }) {
    ...
    <section className={styles.work} id="work" ref={root}>
      ...
      <div className={styles.stage}>
        <div className={styles.track}>
          {projects.map((p, i) => (
            <article className={styles.card} key={p.slug || p.id} style={{ zIndex: 100 - i }}>
              <a className={styles.inner} href={p.slug ? `/work/${p.slug}` : (p.project_url || '#')} target={p.slug ? undefined : "_blank"} rel={p.slug ? undefined : "noreferrer"}>
                <div
                ...
                <div className={styles.meta}>
                  <h3>{p.title ? p.title : L(lang, p, "title")}</h3>
                  <p className={styles.contribution}>{p.short_description ? p.short_description : L(lang, p, "contribution")}</p>
                  <p className={styles.tags}>
                    {p.technology_used 
                      ? p.technology_used.split(',').map((t: string) => t.trim().toUpperCase()).join(" · ")
                      : ((p.fr && lang === "fr" ? p.fr.tags ?? p.tags : p.tags) || []).join(" · ").toUpperCase()}
                  </p>
              ...
              {/* verified destination — a sibling of the card link, so the
                  ...
                  site and GitHub can now both be displayed simultaneously. */}
              <div className={styles.chipContainer}>
                {(p.site || p.project_url) && (
                  <a
                    className={styles.siteChip}
                    href={p.site ? p.site.url : p.project_url}
                    target="_blank"
                    rel="noreferrer"
                    aria-label={`${p.site ? p.site.label : "Live Site"} ↗`}
                  >
                    {p.site ? p.site.label : "Live Site"} <i aria-hidden="true">↗</i>
                  </a>
                )}
                {(p.repo || p.github_url) && (
                  <a
                    className={styles.siteChip}
                    href={p.repo ? p.repo : p.github_url}
                    target="_blank"
                    rel="noreferrer"
                    aria-label="GitHub ↗"
                  >
